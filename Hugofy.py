...
def setvars():
    global settings, path, sitename, repository, hugo_path
...
class HugonewsiteCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        setvars()
        process = hugo_path + " new site " + os.path.join(path, sitename)
...
class HugoversionCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        setvars()
        try:
            out = subprocess.check_output([hugo_path, "version"], stderr=subprocess.STDOUT, universal_newlines=True)
...
class HugoserverstopCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        ...
        try:
            stopCmd1 = "ps aux "
            stopCmd2 = " grep '" + hugo_path + " server' "
...
class HugobuildCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        try:
            buildCmd = hugo_path + " --buildDrafts"
